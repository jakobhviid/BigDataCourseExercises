...
**Note:** This may take several minutes. To keep track of the progress you can either run `kubectl get pods -n meta -w` or an alternative cmd like `watch -n1 kubectl get pods -n meta` in a secondary terminal session. The latter requires an `watch` executable on your localhost.
...
**Note:** This may take several minutes. To keep track of the progress you can either run `kubectl get pods -n meta -w` or an alternative cmd like `watch -n1 kubectl get pods -n meta` in a secondary terminal session. 
